...
package net.mcreator.theshademod.world.dimension;
...
import net.mcreator.theshademod.procedures.PlayersicknessProcedure;
import net.mcreator.theshademod.procedures.PlayerLeavesDimensionProcedure;
import net.mcreator.theshademod.item.ShadowplaneItem;
import net.mcreator.theshademod.block.ShadowstoneBlock;
import net.mcreator.theshademod.block.ShadedimensionframeBlock;
...
public class ShadowplaneDimension extends TheShadeModModElements.ModElement {
	@ObjectHolder("the_shade_mod:shadowplane")
	public static final ModDimension dimension = null;
	@ObjectHolder("the_shade_mod:shadowplane_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public ShadowplaneDimension(TheShadeModModElements instance) {
		super(instance, 19);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("shadowplane"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("the_shade_mod:shadowplane")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("the_shade_mod:shadowplane"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("the_shade_mod:shadowplane"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ShadowplaneItem().setRegistryName("shadowplane"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("shadowplane_portal");
		}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = ShadowplaneDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos)) || !(this.world.getBlockState(blockpos.down())
							.getBlock() == ShadedimensionframeBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == ShadedimensionframeBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == ShadedimensionframeBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("shadowplane_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "shadowplane_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = ShadowplaneDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag
											? ShadedimensionframeBlock.block.getDefaultState().getBlock().getDefaultState()
											: Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, ShadedimensionframeBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
	...
	public static class CustomDimension extends Dimension {
		...
		public Vec3d getFogColor(float cangle, float ticks) {
			return new Vec3d(0.160784313725, 0.109803921569, 0.196078431373);
		}
		...
		if (event.getFrom() == type) {
			{
				...
				PlayerLeavesDimensionProcedure.executeProcedure($_dependencies);
			}
		}
		if (event.getTo() == type) {
			{
				Map<String, Object> $_dependencies = new HashMap<>();
				$_dependencies.put("entity", entity);
				PlayersicknessProcedure.executeProcedure($_dependencies);
			}
		}
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return ShadowstoneBlock.block.getDefaultState();
				}

				public BlockState getDefaultFluid() {
					return BloodwaterBlock.block.getDefaultState();
				}
			});
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(ShadowstoneBlock.block.getDefaultState().getBlock(),
